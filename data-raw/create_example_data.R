...
# 👀 Model Overview ----
# Set up 3 model scenarios with varying timestep
param_scenarios <- data.frame(model_index = c(1,2,3), N_m = c(1,6,12))
...
for (i in 1:length(param_scenarios$model_index)){
  ...
  dd_out[[i]] <- run_DDUST(data, parameters, map, MCMC = TRUE, iter = 100)

}

usethis::use_data(data, parameters, map, dd_out, overwrite = TRUE)

dd_out_transposed <- purrr::list_transpose(dd_out)
dd_mle <- dd_out_transposed$dd_mle
dd_mcmc <- dd_out_transposed$dd_mcmc

usethis::use_data(dd_mle, dd_mcmc, overwrite = TRUE)
